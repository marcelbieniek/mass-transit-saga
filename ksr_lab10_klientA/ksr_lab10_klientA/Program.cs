...
namespace ksr_lab10_klientA
{
    static class Klient
    {
        public const string login = "klient A";
    }
        ...
        public Task Consume(ConsumeContext<IPytanieoPotwierdzenie> ctx)
        {
            if (ctx.Message.login == Klient.login)
            {
                ...
                if (Console.ReadKey().Key == ConsoleKey.Y)
                {
                    ...
                    return ctx.RespondAsync(new Potwierdzenie() { CorrelationId = ctx.Message.CorrelationId });
                }

                Console.WriteLine("Wyslano brak potwierdzenia");
                return ctx.RespondAsync(new BrakPotwierdzenia() { CorrelationId = ctx.Message.CorrelationId });
            }
            return Task.Run(() => { });
        }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var bus = Bus.Factory.CreateUsingRabbitMq(sbc =>
            {
                ...
                sbc.ReceiveEndpoint("sagakolejkaklientA", ep =>
                {
                    ep.Instance(handler);
                });
            });
            ...
            bus.Start();
            Console.WriteLine("start klienta A");
